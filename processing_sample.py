...
from commons.check_json import check_phishing_email, check_phishing_url
...
async def extract_features(text, grammatical_errors):
    email_domains = await extract_emails(text)
    url_domains = await extract_urls(text)
    keywords = ["Urgent action required", "Immediate attention required", "Account suspended", "Account compromised",
                "Security alert", "Emergency", "PayPal", "Bank of America", "Amazon", "Apple", "Microsoft", "IRS",
                "Social Security Administration", "Netflix", "Invoice", "Payment confirmation", "Tax refund",
                "Wire transfer", "Payment overdue", "Unusual activity in your account", "Password reset",
                "Account verification", "Unusual login attempts", "Account locked", "Update your information",
                "Verify your identity", "Social Security number", "Credit card number", "Bank account information",
                "Login credentials", "PIN", "Date of birth", "Mother's maiden name", "Free gift", "Prize winner",
                "Exclusive offer", "Limited time offer", "Discount code", "Special deal", "Click here", "Login now",
                "Verify your account", "Confirm your information", "Reset your password", "Update your account",
                "Dear customer", "Dear valued customer", "Dear user", "Hello", "Hi", "Attached file", "Download file",
                "Click to view document", "View file"]
    keyword_features = [1 if any(keyword.lower() in text.lower() for keyword in keywords) else 0]

    url_patterns = ['http://', 'https://', '.com']
    url_features = [1 if any(pattern in text for pattern in url_patterns) else 0]

    special_characters = ['@', '#', '$', '%', '^', '&', '*', '(', ')']
    special_char_features = [1 if any(char in text for char in special_characters) else 0]
